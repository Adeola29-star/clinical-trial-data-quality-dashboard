...
- **The cause of the incomplete-visit rate is unknown.** Each subject was randomly assigned 3, 4, or 5 completed visits to simulate real-world variation, but in an actual trial an incomplete visit record can result from several different causes which might include a genuine dropout, a rescheduled appointment, a data-entry delay, or a record simply not yet submitted by the site. Before treating any incomplete case as a dropout, the underlying cause would need to be investigated, since the correct follow-up action differs by cause.
